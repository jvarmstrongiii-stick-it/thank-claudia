...
2. **ALWAYS bump `APP_REV`** on every single commit — it's `const APP_REV = NNN;` near the top of `claudia.html` (currently **208**). This is the visible version badge on the dashboard. Forgetting it means Jack sees a stale revision number and thinks deploys aren't working.
...
|-------|---------|--------|
| **Bump APP_REV every commit** | Critical | `const APP_REV = NNN;` near top of claudia.html. Currently **208**. Forgetting this makes Jack think the deploy failed — he sees the old revision number on the dashboard. Increment by 1 every commit, no exceptions. |
...
| **CREW CHECK's clock-out path defaults the teammate's destination to the shop** | Medium | `clockOut` fires `LeavingCrewCheck` with `newJobId: shopJob?.id` — answering YES marks the teammate In Route **to the shop**, whatever their real destination. Since r205 the prompt at least names the destination ("coming with you to X?"). Jack reports this prompt never actually appears in practice (July 2026) — verify against raw data (`job_notes` containing `— with`) before building on it; if it truly never fires, the both-On-Site-same-job condition never occurring is itself a symptom worth investigating. |
...
- **Pause/resume (r208):** `'Paused'` is a third `job_crew.status` value (alongside In Route/On Site) and a third `clockSession.status`. Pausing (⏸ PAUSE in both clock-bar copies, shown only when On Site; reason picked in `PauseSheet` — lunch/materials/washy washy/other) goes through the normal `fieldTimeConfirm` rewind (`type:'pause'`/`'resume'`, all 3 sheet copies). `pauseClock(reason, at)`: banks the leg (`addTodayMs`), writes `Status → Paused (Name) — reason` (which **closes** the current timeline period — 'Paused' is not in `CREW_FIELD_STATUSES`, so paused time counts zero in every report with no special-casing), upserts the crew row to `Paused` at `at`, and sets session `{ status:'Paused', prevStatus, pausedReason, pausedAt, legStartedAt: at }`. `resumeClock(at)` writes `Status → On Site (Name) — back from reason` (new period), restores the crew row, resets `legStartedAt`. **Three invariants:** (1) any `.in('status', […])` query over `job_crew` MUST include `'Paused'` — `reconcileClockFromCrew` and `fetchActiveCrewAssignments` do; omitting it makes reconcile read a paused tech as clocked-out-elsewhere and wipe their session within 60s. (2) `clockOut`/`moveToInRoute` skip their `addTodayMs` banking when leaving from `Paused` (the leg was banked at pause — banking again would count the break). (3) `ClockTimer` takes a `paused` prop that zeroes the live leg so the total freezes. Crew-check prompts (`stillOnSite`) filter `=== 'On Site'` so paused teammates are never auto-marked In Route by someone leaving. `staleClock` App state `{ session, sinceDate }` shows `StaleClockSheet`. As of r175 it is set by `reconcileClockFromCrew` (mount/switch + 60s + focus), which judges staleness by the user's `job_crew` row `status_at` (NOT the local `startedAt`, which is preserved across job moves and drifts). If there's no active crew row the local session is cleared silently (clocked out elsewhere) — no prompt. `clockOutStale(atISO)` closes the session and writes the `Status → Cleared` note at the chosen time (for HoursReport accuracy) but does **not** touch `addTodayMs` (that bucket is today's; the prior day's hours live in `job_notes`). `keepStaleClock()` re-anchors `startedAt`/`legStartedAt` to now so the day starts clean and the prompt won't re-fire.
