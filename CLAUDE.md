...
2. **ALWAYS bump `APP_REV`** on every single commit — it's `const APP_REV = NNN;` near the top of `claudia.html` (currently **200**). This is the visible version badge on the dashboard. Forgetting it means Jack sees a stale revision number and thinks deploys aren't working.
...
|-------|---------|--------|
| **Bump APP_REV every commit** | Critical | `const APP_REV = NNN;` near top of claudia.html. Currently **200**. Forgetting this makes Jack think the deploy failed — he sees the old revision number on the dashboard. Increment by 1 every commit, no exceptions. |
...
| **Time Log periods can lack a closing note** | Medium | `buildCrewTimeline` (line ~311) derives each period's `end`/`endId` from the *next* `Status →` note on the same job; the last recorded period for a user has `endId=null` if nothing closed it. Several office-triggered crew moves (`routeCrewMember`, `answerTravelCheck` in `JobDetail`) clear a person's crew status without writing a `Status → Cleared` note on the job they're leaving, so this isn't rare. Before r190 the TIME LOG editor (`JobDetail`'s `saveLogEdit`/END input) only showed an END field when `endId` already existed, so those periods couldn't be closed out at all. Fixed: END input always shows when the row isn't `ongoing`; `saveLogEdit` inserts a new `Status → Cleared (...) (edited)` note when `endId` is missing, instead of only ever updating an existing one. |
