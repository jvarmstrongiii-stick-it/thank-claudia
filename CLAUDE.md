...
2. **ALWAYS bump `APP_REV`** on every single commit — it's `const APP_REV = NNN;` near the top of `claudia.html` (currently **181**). This is the visible version badge on the dashboard. Forgetting it means Jack sees a stale revision number and thinks deploys aren't working.
...
|-------|---------|--------|
| **Bump APP_REV every commit** | Critical | `const APP_REV = NNN;` near top of claudia.html. Currently **181**. Forgetting this makes Jack think the deploy failed — he sees the old revision number on the dashboard. Increment by 1 every commit, no exceptions. |
...
- **Chore ordering + voice reminders (r181):** `sortChoresForUser(list, userId)` is the single sort applied everywhere open chores are listed (the ⚑ CHORES screen and the new post-clock-in gate): your voice reminders (`kind==='reminder' && assigned_to===userId`) → your other assigned chores → unassigned → everyone else's (shown last, never hidden). `kind` reads the existing `chores.priority` free-text column (already used for `'billing'`) — `'reminder'` is a new convention value, no schema change. Voice: saying "remind me/myself to…" (regex on the raw transcript, not the LLM's paraphrase) auto-assigns the chore to the speaker and tags it `kind:'reminder'`; naming someone else ("remind Brett to…") still assigns to them normally, not tagged as a reminder. `addChore`/`deleteChore` are App-level functions (matching `toggleChore`/`editChore`) shared by both `ChoresView` call sites — don't reintroduce inline closures at a second call site.
- **Post-clock-in chores gate (r181):** `showChoresGate` App state — `clockIn()` sets it when `chores.some(c => !c.done)` (skipped if there are zero open chores). Rendered as an early-return alongside `showHours`/`showMaterials`, reusing `ChoresView` itself (new optional `title`/`ctaLabel` props add a header title override and a sticky bottom CONTINUE button) rather than a second chores UI. `clockIn()` still sets `selectedId` as before, so dismissing the gate lands on the job just clocked into. Only the initial `clockIn()` triggers it — `moveToInRoute`/`flipToOnSite` (mid-shift transitions) do not.
