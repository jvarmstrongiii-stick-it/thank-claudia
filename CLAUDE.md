...
2. **ALWAYS bump `APP_REV`** on every single commit — it's `const APP_REV = NNN;` near the top of `claudia.html` (currently **178**). This is the visible version badge on the dashboard. Forgetting it means Jack sees a stale revision number and thinks deploys aren't working.
...
|-------|---------|--------|
| **Bump APP_REV every commit** | Critical | `const APP_REV = NNN;` near top of claudia.html. Currently **178**. Forgetting this makes Jack think the deploy failed — he sees the old revision number on the dashboard. Increment by 1 every commit, no exceptions. |
| **Shop JobDetail is a stripped variant** | Low | `const isShop` (JobDetail, `job.customer===TMC_SHOP_NAME`) hides JOB VALUE, SCHEDULED, ORIGINAL ASK, NOTES and lifts the shared `materialsSection` up under TODAY'S TIME (r177). The materials block is a single `materialsSection` const rendered `{isShop && …}` at top or `{!isShop && …}` in the normal spot — edit it once, not twice. |
